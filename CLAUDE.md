...
8. **Color carries meaning only.** `--up`/`--down` on % changes and votes exclusively (including the stat-card tints that report those same signs); the lavender accent on interaction only. *Amended 2026-07-17: the card system adopted in DESIGN.md (per the 2026-07-15 decision) supersedes this rule's former "no cards or rounded containers" clause — bordered cards and the two sanctioned radii are allowed as DESIGN.md §Structure specifies. Shadows remain banned. The color-meaning clause is unchanged.* (DESIGN.md)
